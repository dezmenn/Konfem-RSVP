...
function getGuestService() {
    if (!guestService) {
        ...
        if (process.env.SKIP_DB_SETUP === 'true') {
            guestService = new MockGuestService_1.MockGuestService();
        }
        else {
            const guestRepository = new GuestRepository_1.GuestRepository();
            guestService = new GuestService_1.GuestService(guestRepository);
        }
    }
    return guestService;
}
...
router.get('/:eventId/search', async (req, res) => {
    try {
        ...
        const filters = {
            ...
            brideOrGroomSide: brideOrGroomSide,
            search: search
        };
...
router.post('/:eventId/import/csv/preview', upload.single('csvFile'), async (req, res) => {
    try {
        const { eventId } = req.params;
        if (!req.file) {
...
router.post('/:eventId/import/contacts', async (req, res) => {
    try {
        const { eventId } = req.params;
...
router.post('/:id/assign-table', async (req, res) => {
    try {
        const { id } = req.params;
        const { tableId } = req.body;
        if (!tableId) {
...
router.post('/:id/unassign-table', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await getGuestService().unassignGuestFromTable(id);
        if (!result.success) {
