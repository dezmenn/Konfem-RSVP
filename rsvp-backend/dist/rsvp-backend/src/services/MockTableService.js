...
class MockTableService {
    ...
    async assignGuestToTable(guestId, tableId) {
        ...
        allTables.forEach(existingTable => {
            if (existingTable.assignedGuests && existingTable.assignedGuests.includes(guestId)) {
                const updatedAssignedGuests = existingTable.assignedGuests.filter((id) => id !== guestId);
        ...
        const guestAppearances = verificationTables.filter(t => t.assignedGuests && t.assignedGuests.includes(guestId));
        if (guestAppearances.length > 1) {
            ...
            verificationTables.forEach(t => {
                if (t.id !== tableId && t.assignedGuests && t.assignedGuests.includes(guestId)) {
                    const cleanedGuests = t.assignedGuests.filter((id) => id !== guestId);
    ...
    async unassignGuestFromTable(guestId) {
        ...
        allTables.forEach(table => {
            if (table.assignedGuests && table.assignedGuests.includes(guestId)) {
                const updatedAssignedGuests = table.assignedGuests.filter((id) => id !== guestId);
    ...
    async autoArrangeGuests(eventId, options = {}) {
        ...
        try {
            ...
            const guestsToArrange = allGuests.filter(guest => guest.rsvpStatus === 'accepted');
            ...
            const unassignedGuests = guestsToArrange.filter(guest => {
                ...
                const isInTableArray = availableTables.some(table => table.assignedGuests && table.assignedGuests.includes(guest.id));
    ...
    async synchronizeGuestTableAssignments(eventId) {
...
//# sourceMappingURL=MockTableService.js.map