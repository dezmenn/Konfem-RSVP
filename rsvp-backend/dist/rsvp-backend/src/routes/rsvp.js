...
const MockRSVPService_1 = require("../services/MockRSVPService");
const logger_1 = require("../utils/logger");
const router = express_1.default.Router();
// Use mock service in demo mode
const rsvpService = process.env.SKIP_DB_SETUP === 'true'
    ? new MockRSVPService_1.MockRSVPService()
    : new RSVPService_1.RSVPService();
...
router.post('/events/:eventId/invitation-templates', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error creating invitation template:', error);
...
router.get('/events/:eventId/invitation-templates', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching invitation templates:', error);
...
router.get('/events/:eventId/invitation-templates/default', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching default invitation template:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
router.get('/invitation-templates/:id', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching invitation template:', error);
...
router.put('/invitation-templates/:id', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error updating invitation template:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
router.delete('/invitation-templates/:id', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error deleting invitation template:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.post('/guests/:guestId/rsvp-token', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error generating RSVP token:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
router.get('/guests/:guestId/rsvp-link', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error generating RSVP link:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.get('/rsvp/:token', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error validating RSVP token:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
router.post('/rsvp/:token/response', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error submitting RSVP response:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
router.put('/rsvp-responses/:id', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error updating RSVP response:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
router.get('/guests/:guestId/rsvp-response', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching RSVP response:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
router.get('/events/:eventId/rsvp-responses', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching event RSVP responses:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
// Public RSVP Routes
router.post('/events/:eventId/public-rsvp', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error submitting public RSVP:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
router.get('/events/:eventId/public-rsvp', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching public RSVP registrations:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
// Invitation Preview
router.get('/invitation-templates/:templateId/preview/:guestId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error generating invitation preview:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.get('/events/:eventId/rsvp-statistics', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching RSVP statistics:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.get('/events/:eventId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching event:', error);
...
router.get('/tokens/:eventId', async (req, res) => {
    try {
        ...
        if (process.env.SKIP_DB_SETUP === 'true' && rsvpService instanceof MockRSVPService_1.MockRSVPService) {
            const tokens = await rsvpService.getEventRSVPTokens(eventId);
        ...
        logger_1.logger.error('Error fetching event tokens:', error);
        res.status(500).json({ success: false, error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.post('/responses', async (req, res) => {
    try {
        // Only available in demo mode
        if (process.env.SKIP_DB_SETUP === 'true' && rsvpService instanceof MockRSVPService_1.MockRSVPService) {
            const response = await rsvpService.createRSVPResponse(req.body);
        ...
        logger_1.logger.error('Error creating RSVP response:', error);
...
router.get('/responses/guest/:guestId/event/:eventId', async (req, res) => {
    try {
        ...
        // Only available in demo mode
        if (process.env.SKIP_DB_SETUP === 'true' && rsvpService instanceof MockRSVPService_1.MockRSVPService) {
            const response = await rsvpService.getRSVPResponseByGuestAndEvent(guestId, eventId);
            if (!response) {
...
router.post('/public-registration', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error submitting public registration:', error);
        res.status(400).json({ success: false, error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
// Utility Routes
