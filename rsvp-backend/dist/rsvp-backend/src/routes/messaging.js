...
const MockGuestService_1 = require("../services/MockGuestService");
const DemoDataService_1 = require("../services/DemoDataService");
const logger_1 = require("../utils/logger");
const router = express_1.default.Router();
// Use the shared WhatsApp service instance from global
const getWhatsAppService = () => {
    const service = global.whatsAppMockService;
    if (!service) {
...
const demoDataService = DemoDataService_1.DemoDataService.getInstance();
const mockGuestService = new MockGuestService_1.MockGuestService();
...
router.post('/events/:eventId/bulk-invitations', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error starting bulk invitation sending:', error);
...
router.get('/bulk-invitations/:trackingId/progress', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching bulk invitation progress:', error);
...
The Event Team`;
        ...
        const result = await whatsAppService.sendMessage(guest.phoneNumber, invitationContent, messageId);
        if (result.success) {
            res.status(201).json({
                success: true,
                data: {
        ...
        logger_1.logger.error('Error sending individual invitation:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
// Send reminder
router.post('/guests/:guestId/reminder', async (req, res) => {
    try {
        ...
        const whatsAppService = getWhatsAppService();
        const result = await whatsAppService.sendMessage(guest.phoneNumber, content, messageId);
        if (result.success) {
            res.status(201).json({
                success: true,
                data: {
...
router.post('/events/:eventId/schedule-messages', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error scheduling messages:', error);
        res.status(400).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.post('/process-scheduled', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error processing scheduled messages:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.get('/events/:eventId/message-statistics', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching message statistics:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.post('/events/:eventId/retry-failed', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error retrying failed messages:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.get('/guests/:guestId/messages', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching guest messages:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.delete('/messages/:messageId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error cancelling message:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.post('/cleanup-trackers', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error cleaning up trackers:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
