...
if (process.env.SKIP_DB_SETUP === 'true') {
    ...
    tableService = new MockTableService_1.MockTableService();
}
else {
    // Use real service with repositories
    const tableRepository = new TableRepository_1.TableRepository();
    const guestRepository = new GuestRepository_1.GuestRepository();
    const venueElementRepository = new VenueElementRepository_1.VenueElementRepository();
    tableService = new TableService_1.TableService(tableRepository, guestRepository, venueElementRepository);
}
...
router.post('/events/:eventId/auto-arrange-enhanced', async (req, res) => {
    try {
        ...
        if ('autoArrangeGuestsEnhanced' in tableService) {
            const result = await tableService.autoArrangeGuestsEnhanced(eventId, constraints);
            if (result.success) {
                res.json(result);
            }
            else {
            ...
            const result = await tableService.autoArrangeGuests(eventId, legacyOptions);
