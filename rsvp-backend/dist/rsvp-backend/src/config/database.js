...
exports.closeDatabase = exports.getPool = exports.setupDatabase = void 0;
...
const setupDatabase = async () => {
    ...
    if (process.env.NODE_ENV === 'test' || process.env.SKIP_DB_SETUP === 'true') {
        logger_1.logger.info('Database setup skipped (test mode or SKIP_DB_SETUP=true)');
        return;
    }
    // For demo purposes, skip database setup if no database is configured
    if (!process.env.DB_HOST && !process.env.DATABASE_URL) {
        logger_1.logger.info('Database setup skipped (no database configured for demo)');
        return;
    }
    const config = {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            logger_1.logger.warn('Continuing without database connection for demo purposes');
            return;
        }
        throw error;
    }
