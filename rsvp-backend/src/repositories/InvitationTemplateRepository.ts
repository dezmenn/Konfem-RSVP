...
export class InvitationTemplateRepository extends BaseRepository {
  ...
  async setAsDefault(id: string, eventId: string): Promise<InvitationTemplate | null> {
    return this.withTransaction(async (client) => {
      ...
      await client.query(
        'UPDATE invitation_templates SET is_default = false WHERE event_id = $1',
        [eventId]
      );
      
      // Set the specified template as default
      const result = await client.query(
        'UPDATE invitation_templates SET is_default = true, updated_at = $1 WHERE id = $2 AND event_id = $3 RETURNING *',
        [new Date(), id, eventId]
      );
  ...
  private mapRowToInvitationTemplate(row: any): InvitationTemplate {
    return {
      id: row.id,
