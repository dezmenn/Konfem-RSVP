...
export class WebSocketClient {
  ...
  private setupNetworkListener(): void {
    // Note: NetInfo would need to be installed for React Native
  ...
  public connect(eventId: string, userId?: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      // Use localhost for development, would be configurable in production
      ...
      this.socket.on('registered', (data) => {
        console.log('Mobile WebSocket registration confirmed:', data);
      });

      this.socket.on('sync_event', (syncEvent: SyncEvent) => {
        console.log('Mobile received sync event:', syncEvent);
        this.handleSyncEvent(syncEvent);
      });

      this.socket.on('sync_data', (syncData: SyncData) => {
        console.log('Mobile received sync data:', syncData);
      ...
      this.socket.on('conflict_resolved', (conflictData: ConflictData) => {
        console.log('Mobile conflict resolved:', conflictData);
  ...
  private attemptReconnect(): void {
    ...
    setTimeout(() => {
      if (this.eventId) {
        this.connect(this.eventId, this.userId).catch(error => {
  ...
  public shouldSync(): boolean {
    return this.isConnected && this.eventId !== null && this.isOnline;
  }
