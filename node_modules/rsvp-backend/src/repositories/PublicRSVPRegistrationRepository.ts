...
import { PublicRSVPRegistration, AdditionalGuestDetail } from '../../../shared/src/types';
import { PublicRSVPRegistrationInput, PublicRSVPRegistrationUpdate } from '../models/PublicRSVPRegistration';

export class PublicRSVPRegistrationRepository extends BaseRepository {
  async create(registrationData: PublicRSVPRegistrationInput): Promise<PublicRSVPRegistration> {
    const query = `
  ...
  async findById(id: string): Promise<PublicRSVPRegistration | null> {
  ...
  async findByEventId(eventId: string): Promise<PublicRSVPRegistration[]> {
  ...
  async findByPhoneNumber(eventId: string, phoneNumber: string): Promise<PublicRSVPRegistration | null> {
  ...
  async update(id: string, updates: PublicRSVPRegistrationUpdate): Promise<PublicRSVPRegistration | null> {
  ...
  async getEventStatistics(eventId: string): Promise<{
    ...
    if (row.all_relationship_types) {
      row.all_relationship_types.forEach((type: string) => {
        if (type) {
  ...
  private mapRowToPublicRSVPRegistration(row: any): PublicRSVPRegistration {
    return {
      id: row.id,
