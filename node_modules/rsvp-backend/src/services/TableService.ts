...
import { TableInput, TableUpdate, TableModel } from '../models/Table';
...
export class TableService {
  ...
  async createTable(tableData: TableInput): Promise<Table> {
    // Validate input
    const validation = TableModel.validate(tableData);
    if (!validation.isValid) {
      throw new Error(`Validation failed: ${validation.errors.join(', ')}`);
    }

    // Sanitize input
    const sanitizedData = TableModel.sanitize(tableData);
  ...
  async updateTable(id: string, updates: TableUpdate): Promise<Table> {
    // Validate updates
    const validation = TableModel.validateUpdate(updates);
    if (!validation.isValid) {
      throw new Error(`Validation failed: ${validation.errors.join(', ')}`);
    }
  ...
  private checkTablePositionConflicts(position: Position, existingTables: Table[]): string[] {
