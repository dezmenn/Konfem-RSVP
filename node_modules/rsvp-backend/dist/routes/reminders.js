...
const MockEventService_1 = require("../services/MockEventService");
const MockGuestRepository_1 = require("../services/MockGuestRepository");
...
function getReminderService() {
    if (!reminderService) {
        ...
        const globalWhatsAppService = global.whatsAppMockService;
        let whatsAppService;
        if (globalWhatsAppService) {
            ...
            whatsAppService = new WhatsAppMockService_1.WhatsAppMockService(messageRepository);
        }
        const messagingService = new MessagingService_1.MessagingService(whatsAppService);
        // Use mock services in demo mode
        if (process.env.SKIP_DB_SETUP === 'true') {
            const mockEventService = new MockEventService_1.MockEventService();
            const mockGuestRepository = new MockGuestRepository_1.MockGuestRepository();
            reminderService = new ReminderService_1.ReminderService(messagingService, mockEventService, mockGuestRepository);
        }
        else {
            reminderService = new ReminderService_1.ReminderService(messagingService);
        }
    }
...
router.post('/configure', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error configuring reminders:', error);
        res.status(500).json({
...
router.get('/event/:eventId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error getting reminder schedules:', error);
        res.status(500).json({
...
router.put('/schedule/:scheduleId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error updating reminder schedule:', error);
        res.status(500).json({
...
router.delete('/schedule/:scheduleId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error deleting reminder schedule:', error);
        res.status(500).json({
...
router.post('/schedule/:scheduleId/toggle', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error toggling reminder schedule:', error);
        res.status(500).json({
...
router.get('/status/:eventId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error getting reminder status:', error);
        res.status(500).json({
...
router.post('/execute/:scheduleId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error executing reminders:', error);
        res.status(500).json({
...
router.post('/execute-all/:eventId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error executing all reminders:', error);
        res.status(500).json({
...
router.get('/executions/:eventId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error getting reminder executions:', error);
        res.status(500).json({
...
router.post('/defaults/:eventId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error creating default reminders:', error);
        res.status(500).json({
...
router.get('/statistics/:eventId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error getting reminder statistics:', error);
        res.status(500).json({
...
router.get('/template/default', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error getting default template:', error);
        res.status(500).json({
