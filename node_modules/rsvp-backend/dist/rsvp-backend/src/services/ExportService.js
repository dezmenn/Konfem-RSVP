...
class ExportService {
    constructor(guestRepository, tableRepository, venueElementRepository) {
        this.guestRepository = guestRepository;
        this.tableRepository = tableRepository;
        this.venueElementRepository = venueElementRepository;
    }
    async exportSeatingChart(eventId, options) {
        try {
    ...
    async exportGuestList(eventId, format) {
        try {
    ...
    async exportVenueLayout(eventId, format) {
        try {
            const seatingData = await this.gatherSeatingChartData(eventId);
            if (format === 'pdf') {
    ...
    async gatherSeatingChartData(eventId) {
        ...
        const occupiedSeats = tablesWithGuests.reduce((sum, table) => sum + table.guests.reduce((guestSum, guest) => guestSum + 1 + guest.additionalGuestCount, 0), 0);
            ...
            data.tables.forEach(table => {
                if (table.guests.length === 0) {
                    ...
                    table.guests.forEach(guest => {
                        ...
                        csvLines.push(`"${table.name}","${guest.name}",${guest.additionalGuestCount},${1 + guest.additionalGuestCount},"${dietaryRestrictions}","${specialRequests}",${table.position.x},${table.position.y}`);
                    });
                }
            });
    ...
    async exportGuestListToCSV(guestData) {
        try {
    ...
    async exportGuestListToExcel(guestData) {
        try {
    ...
    async exportVenueLayoutToPDF(data) {
        try {
    ...
    async exportVenueLayoutToExcel(data) {
        try {
